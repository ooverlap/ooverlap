...
# OOVERLAP_FORCE_ALL_GATHER_SLOT_PATCH_V1
...
class CudaCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        self.force_allreduce_backend = os.getenv(
            "VLLM_FORCE_ALLREDUCE_BACKEND", "auto"
        ).strip().lower()
        allowed_force_backends = {"auto", "ooverlap", "custom", "pynccl", "nccl_symm"}
        if self.force_allreduce_backend not in allowed_force_backends:
            raise ValueError(
                "VLLM_FORCE_ALLREDUCE_BACKEND must be one of "
                f"{sorted(allowed_force_backends)}, got {self.force_allreduce_backend!r}"
            )
        ...
        from vllm.distributed.device_communicators.ooverlap_all_reduce import (
            OoverlapAllReduce,
        )

        self.pynccl_comm: PyNcclCommunicator | None = None
        if self.world_size > 1 and self.force_allreduce_backend != "ooverlap":
        ...
        self.ooverlap_comm: OoverlapAllReduce | None = None

        if (
            self.force_allreduce_backend == "auto"
            and use_torch_symm_mem
        ...
        if (
            self.force_allreduce_backend == "auto"
            and self.use_flashinfer_allreduce
        ...
        if (
            self.force_allreduce_backend in ("auto", "custom")
        ...
        if (
            self.force_allreduce_backend == "ooverlap"
            and self.world_size > 1
            and current_platform.is_cuda()
        ):
            self.ooverlap_comm = OoverlapAllReduce(
                group=self.cpu_group,
                device=self.device,
    ...
    def _log_all_reduce_backend_selection(self) -> None:
        ...
        if self.ooverlap_comm is not None and not self.ooverlap_comm.disabled:
            enabled_ar_backends.append("OOVERLAP")
        ...
        logger.info_once(
            ...
            "'%s' out of potential backends: %s. Forced backend: %s.",
            "[" + ", ".join(f"'{b}'" for b in enabled_ar_backends) + "]",
            self.unique_name or "<unnamed>",
            "[" + ", ".join(f"'{b}'" for b in all_potential_ar_backends) + "]",
            self.force_allreduce_backend,
            scope="global",
        )

    def _all_reduce_forced(self, input_, backend: str):
        if backend == "ooverlap":
            ooverlap_comm = self.ooverlap_comm
            if (
                ooverlap_comm is None
                or ooverlap_comm.disabled
                or not ooverlap_comm.should_ooverlap_ar(input_)
            ):
                raise RuntimeError(
                    "Forced ooverlap all-reduce backend is unavailable or rejected "
                    f"tensor shape={tuple(input_.shape)} dtype={input_.dtype} "
                    f"device={input_.device} contiguous={input_.is_contiguous()}"
                )
            out = ooverlap_comm.all_reduce(input_)
        ...
        if backend == "custom":
            ...
            if ca_comm is None or ca_comm.disabled or not ca_comm.should_custom_ar(input_):
                raise RuntimeError(
                    "Forced vLLM CustomAllreduce backend is unavailable or rejected "
                    f"tensor shape={tuple(input_.shape)} dtype={input_.dtype} "
                    f"numel={input_.numel()} nbytes={input_.numel() * input_.element_size()}"
                )
        ...
        if backend == "pynccl":
            ...
            if pynccl_comm is None or pynccl_comm.disabled:
                raise RuntimeError("Forced vLLM PyNCCL backend is unavailable")
            out = pynccl_comm.all_reduce(input_)
            if out is None:
                raise RuntimeError("Forced vLLM PyNCCL backend returned None")
            return out

        if backend == "nccl_symm":
            try:
                out = torch.ops.vllm.all_reduce_symmetric_with_copy(input_)
            except Exception as exc:
                raise RuntimeError(
                    "Forced vLLM NCCL symmetric helper failed. "
                    "Make sure VLLM_USE_NCCL_SYMM_MEM=1 and the op is registered."
                ) from exc
            if out is None:
                raise RuntimeError("Forced vLLM NCCL symmetric helper returned None")
            return out

        raise RuntimeError(f"Unknown forced all-reduce backend: {backend!r}")

    def all_reduce(self, input_):
        if self.force_allreduce_backend != "auto":
            return self._all_reduce_forced(input_, self.force_allreduce_backend)
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        if self.force_allreduce_backend == "ooverlap":
            ooverlap_comm = self.ooverlap_comm
            if ooverlap_comm is None or ooverlap_comm.disabled:
                raise RuntimeError(
                    "Forced ooverlap all-gather backend is unavailable"
                )
            # Strict path: the adapter raises for unsupported dimensions,
            # dtypes, layouts, devices, or output sizes. Never fall back to
            # PyNccl, torch.distributed, or the base communicator here.
            return ooverlap_comm.all_gather(input_, dim)

        # Route uniform dim-0 all-gathers through NVLS symmetric memory when
        # enabled (mirrors reduce_scatter); otherwise use the normal vLLM path.
        if dim < 0:
            dim += input_.dim()
    ...
    def destroy(self):
        ...
        if self.ooverlap_comm is not None:
            self.ooverlap_comm.destroy()
            self.ooverlap_comm = None
