...
def main():
    parser = argparse.ArgumentParser("Persistent 4-GPU all-reduce smoke + benchmark")
    ...
    parser.add_argument("--dev2", type=int, default=2)
    parser.add_argument("--dev3", type=int, default=3)
    ...
    print(
        f"[info] dev0={args.dev0} dev1={args.dev1} dev2={args.dev2} dev3={args.dev3}"
    )
    ...
    if args.mode in ("smoke", "both"):
        ok = ext.tma_persistent_four_gpu_allreduce_smoke_test(
            int(args.numel),
            int(args.dev0),
            int(args.dev1),
            int(args.dev2),
        ...
        print(f"[result] persistent 4-gpu smoke: {ok}")
        assert ok is True

    if args.mode in ("bench", "both"):
        metrics = ext.benchmark_persistent_four_gpu_allreduce_sm90(
            int(args.numel),
            int(args.iters),
            int(args.warmup),
            int(args.dev0),
        ...
        print_metrics("Persistent 4-GPU all-reduce vs basic vs NCCL", metrics)

    torch.cuda.synchronize(args.dev0)
    torch.cuda.synchronize(args.dev1)
    torch.cuda.synchronize(args.dev2)
    torch.cuda.synchronize(args.dev3)
    print("PASS ✅ persistent 4-GPU all-reduce path")
