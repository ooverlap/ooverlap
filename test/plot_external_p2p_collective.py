...
COLLECTIVE_TITLES = {'allreduce': 'All-Reduce', 'reduce_scatter': 'Reduce-Scatter', 'all_gather': 'All-Gather'}
PLOT_LINEWIDTH = 1.8
PLOT_MARKERSIZE = 6.5
...
def infer_tp(path: Path) -> Optional[int]:
    for part in reversed(path.parts):
        match = re.fullmatch('tp(\\d+)', part.lower())
        if match:
...
def load_rows(path: Path) -> List[Dict[str, object]]:
    ...
    with path.open('r', encoding='utf-8', newline='') as handle:
        ...
        if missing:
            raise PlotInputError(f"{path} is missing required columns: {', '.join(missing)}")
        for raw in reader:
...
def group_rows(rows: List[Dict[str, object]], metric: str) -> Dict[str, Dict[Optional[int], List[Dict[str, object]]]]:
    grouped: Dict[str, Dict[Optional[int], List[Dict[str, object]]]] = {collective: {} for collective in COLLECTIVES}
    for row in rows:
...
def plot_tp_group(
    ...
    *,
    ...
    for row_idx, (metric, t_ccl_key, nccl_key, symmetric_key, ylabel) in enumerate(METRIC_SPECS):
...
def plot_combined(rows: List[Dict[str, object]], tp: int, output_path: Path) -> None:
    del tp
...
def plot_paired_tp(
    ...
    for group_idx in range(2):
        inner = outer[group_idx].subgridspec(nrows=2, ncols=3, wspace=0.16, hspace=0.22)
        group_axes.append([[fig.add_subplot(inner[row_idx, col_idx]) for col_idx in range(3)] for row_idx in range(2)])
    legend_by_label: Dict[str, object] = {}
    plot_tp_group(left_rows, group_axes[0], legend_by_label, show_ylabels=False)
    plot_tp_group(right_rows, group_axes[1], legend_by_label, show_ylabels=False)
    fig.subplots_adjust(left=0.045, right=0.995, bottom=0.17, top=0.8)
...
def configure_paper_y_axis(axis, values: Iterable[float]) -> None:
    ...
    major_ticks = sorted({float(tick) for tick in axis.get_yticks() if math.isfinite(float(tick))})
    if len(major_ticks) < 2:
        return
    positive_steps = [right - left for left, right in zip(major_ticks, major_ticks[1:]) if right - left > 0.0]
    if not positive_steps:
        return
    step = min(positive_steps)
    epsilon = 1e-10 * max(1.0, abs(min_value), abs(max_value))
    lower_tick = next((tick for tick in reversed(major_ticks) if tick < min_value - epsilon), None)
    if lower_tick is None:
        lower_tick = major_ticks[0]
        while lower_tick >= min_value - epsilon:
            lower_tick -= step
        major_ticks.append(lower_tick)
    upper_tick = next((tick for tick in major_ticks if tick > max_value + epsilon), None)
...
def plot_paired_tp_3x4(
    ...
    """
    tp_groups = sorted(((int(left_tp), left_rows), (int(right_tp), right_rows)), key=lambda item: item[0])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Plot one external-P2P TP sweep or combine two TP sweeps in one figure'
    )
    parser.add_argument('--text', default=None, help='Single-TP sweep tab-separated text file.')
    parser.add_argument(
        '--tp',
        type=int,
        default=None,
    ...
    parser.add_argument('--left-text', default=None, help='Left TP-group sweep text file for paired mode.')
    parser.add_argument(
        '--left-tp',
        type=int,
        default=None,
    ...
    parser.add_argument('--right-text', default=None, help='Right TP-group sweep text file for paired mode.')
    parser.add_argument(
        '--right-tp',
        type=int,
        default=None,
...
def resolve_tp(path: Path, explicit_tp: Optional[int], option_name: str) -> int:
    ...
    if tp is None or tp < 2:
        raise PlotInputError(f'{option_name} must be at least 2 or inferable from a tpN path')
    return tp
...
def main() -> int:
    ...
    if single_options_used and paired_options_used:
        raise PlotInputError('use either --text/--tp or --left-text/--right-text, not both')
    if args.text is not None:
        text_path = Path(args.text).expanduser().resolve()
        tp = resolve_tp(text_path, args.tp, '--tp')
        out_prefix = Path(args.out_prefix).expanduser().resolve() if args.out_prefix else text_path.with_suffix('')
    ...
    if not paired_options_used:
        raise PlotInputError('provide --text for single mode or both --left-text and --right-text for paired mode')
    if args.left_text is None or args.right_text is None:
        raise PlotInputError('paired mode requires both --left-text and --right-text')
    ...
    plot_function(load_rows(left_path), left_tp, load_rows(right_path), right_tp, Path(f'{out_prefix}.png'))
    return 0
