...
def run_smoke(ext, collective: str, numel: int, dev0: int, dev1: int) -> bool:
    if hasattr(ext, "tma_persistent_two_gpu_collective_smoke_test"):
        return ext.tma_persistent_two_gpu_collective_smoke_test(
            collective,
            int(numel),
            int(dev0),
            int(dev1),
    ...
    if collective == "allreduce" and hasattr(ext, "tma_persistent_two_gpu_allreduce_smoke_test"):
        return ext.tma_persistent_two_gpu_allreduce_smoke_test(
            int(numel),
            int(dev0),
            int(dev1),
        )
    ...
    raise AttributeError(
        "Extension does not expose tma_persistent_two_gpu_collective_smoke_test. "
        "Rebuild after adding the new pybind binding."
    )
...
def run_benchmark(
    ...
    if hasattr(ext, "benchmark_persistent_two_gpu_collective_sm90"):
        return ext.benchmark_persistent_two_gpu_collective_sm90(
            collective,
            int(numel),
            int(iters),
            int(warmup),
    ...
    if collective == "allreduce" and hasattr(ext, "benchmark_persistent_two_gpu_allreduce_sm90"):
        return ext.benchmark_persistent_two_gpu_allreduce_sm90(
            int(numel),
            int(iters),
            int(warmup),
            int(dev0),
    ...
    raise AttributeError(
        "Extension does not expose benchmark_persistent_two_gpu_collective_sm90. "
        "Rebuild after adding the new pybind binding."
    )
...
def main():
    parser = argparse.ArgumentParser(
        "Persistent 2-GPU collective smoke + benchmark"
    )
    ...
    if args.mode in ("smoke", "both"):
        ...
        print(f"[result] persistent {display_collective} smoke: {ok}")
        assert ok is True
    ...
    if args.mode in ("bench", "both"):
        ...
        print_metrics(
            f"Persistent 2-GPU {display_collective} vs NCCL",
            metrics,
        )
    ...
    print(f"PASS ✅ persistent 2-GPU {display_collective} path")
