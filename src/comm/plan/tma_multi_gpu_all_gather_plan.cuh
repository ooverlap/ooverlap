...
__host__ __device__ __forceinline__ comm::task::WindowTask make_all_gather_copy_task(
    ...
    int window_chunks,
    bool terminal = false) {
    if (multi_gpu_all_gather_use_fast_copy(plan_kind)) {
...
bool build_tma_multi_gpu_all_gather_naive_plan(
    ...
    *out_num_blocks = 0;
    ...
    if (peer_count < 0 || peer_count > kTmaMultiGpuAllGatherMaxPeers) {
        return false;
    }
    ...
    const int tasks_per_cta =
        naive_multi_gpu_all_gather_tasks_per_cta(
            peer_count,
            out_of_place);
