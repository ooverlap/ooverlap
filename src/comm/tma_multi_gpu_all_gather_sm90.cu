...
bool dtype_supported_for_all_gather(oo_dtype_t dtype) {
...
template <
    ...
    int MaxTasks,
    int MaxPeers>
__global__ void all_gather_window_task_executor_kernel_sm90(
    ...
    /*
     * The executor template currently needs a ReduceApply type even when the
     * plan only contains copy tasks. The reduce path is never used by this
     * all-gather plan.
     */
...
template <
    ...
    int MaxTasks,
    int MaxPeers>
cudaError_t launch_all_gather_window_task_executor_sm90(
    comm::plan::WindowTaskExecutorPlan<MaxTasks> plan,
    int num_blocks,
    int threads,
    int* local_ready_signal,
    ...
    all_gather_window_task_executor_kernel_sm90<
        ReduceApply,
        ChunkBytes,
        StageDepth,
        MaxTasks,
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
void configure_all_gather_kernel_once_for(int device) {
    using Variant = comm::TmaPipelineVariant<ChunkBytes, StageDepth>;
    ...
    if (total_smem_bytes >
        ...
        throw std::runtime_error(
            "tma_multi_gpu_all_gather_configure_kernel_once: "
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
cudaError_t launch_all_gather_rank_kernel_sm90(
    const void* local_in,
    void* local_buf,
    ...
    void* peer_slices[comm::plan::kTmaMultiGpuAllGatherMaxPeers] = {};
    ...
    MultiGpuReadySignalPlan<
        comm::plan::kTmaMultiGpuAllGatherMaxPeers> ready_plan{};
    ...
    comm::plan::WindowTaskExecutorPlan<
        comm::plan::kTmaMultiGpuAllGatherMaxWindowTasks> plan{};
    ...
    configure_all_gather_kernel_once_for<
        ReduceApply,
        ElemBytes,
        ChunkBytes,
    ...
    return launch_all_gather_window_task_executor_sm90<
        ReduceApply,
        ChunkBytes,
        StageDepth,
        comm::plan::kTmaMultiGpuAllGatherMaxWindowTasks,
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
cudaError_t launch_all_gather_dtype_sm90(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    int peer_count,
    ...
    return launch_all_gather_rank_kernel_sm90<
        ReduceApply,
        ElemBytes,
        ChunkBytes,
        StageDepth>(
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
void configure_all_gather_dtype_sm90(int device) {
    using Variant = comm::TmaPipelineVariant<ChunkBytes, StageDepth>;
...
cudaError_t dispatch_all_gather_rank_kernel_variant_sm90(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    int peer_count,
    ...
    if (dtype == OO_DTYPE_FLOAT16) {
        return launch_all_gather_dtype_sm90<
...
cudaError_t dispatch_all_gather_rank_kernel_sm90(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    int peer_count,
...
#define OO_TRY_VARIANT(CHUNK_BYTES_VALUE, STAGE_DEPTH_VALUE)                 \
    if (launch_config.chunk_bytes == (CHUNK_BYTES_VALUE) &&                  \
        ...
        return dispatch_all_gather_rank_kernel_variant_sm90<                 \
            (CHUNK_BYTES_VALUE),                                             \
            (STAGE_DEPTH_VALUE)>(                                            \
                local_in,                                                    \
                local_buf,                                                   \
...
template <int ChunkBytes, int StageDepth>
void configure_all_gather_dispatch_variant_sm90(
    oo_dtype_t dtype,
    int device) {
    if (dtype == OO_DTYPE_FLOAT16) {
        configure_all_gather_dtype_sm90<
            comm::pipeline::PipelineReduceAddNoFtzF16,
            static_cast<int>(sizeof(half)),
            ChunkBytes,
            StageDepth>(device);
    ...
    throw std::invalid_argument(
        "tma_multi_gpu_all_gather_configure_kernel_once: unsupported dtype");
}

void configure_all_gather_dispatch_sm90(
    oo_dtype_t dtype,
    int device) {
    configure_all_gather_dispatch_variant_sm90<
...
void tma_multi_gpu_all_gather_configure_kernel_once(
    oo_dtype_t dtype,
    int device) {
    configure_all_gather_dispatch_sm90(dtype, device);
}
...
cudaError_t enqueue_tma_multi_gpu_all_gather_rank_sm90(
    ...
    if (!dtype_supported_for_all_gather(dtype)) {
        return cudaErrorInvalidValue;
    }
    ...
    try {
        return dispatch_all_gather_rank_kernel_sm90(
            local_in,
            local_buf,
            peer_bufs,
            peer_count,
