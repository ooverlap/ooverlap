...
template <
    ...
    int ChunkBytes,
    int StageDepth>
void configure_multi_gpu_kernel_once_for(int device) {
    using Variant = comm::TmaPipelineVariant<ChunkBytes, StageDepth>;
    ...
    if (total_smem_bytes >
        ...
        throw std::runtime_error(
            "tma_multi_gpu_allreduce_configure_kernel_once: "
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
cudaError_t launch_multi_gpu_rank_kernel_sm90(
    const void* local_in,
    void* local_buf,
    ...
    void* peer_slices[comm::plan::kTmaMultiGpuAllreduceMaxPeers] = {};
    ...
    MultiGpuReadySignalPlan<
        comm::plan::kTmaMultiGpuAllreduceMaxPeers> ready_plan{};
    ...
    comm::plan::WindowTaskExecutorPlan<
        comm::plan::kTmaMultiGpuAllreduceMaxWindowTasks> plan{};
    ...
    configure_multi_gpu_kernel_once_for<
        ReduceApply,
        ElemBytes,
        ChunkBytes,
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
cudaError_t launch_multi_gpu_reduce_op_sm90(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    int peer_count,
    ...
    return launch_multi_gpu_rank_kernel_sm90<
        ReduceApply,
        ElemBytes,
        ChunkBytes,
        StageDepth>(
...
template <
    ...
    int ChunkBytes,
    int StageDepth>
void configure_multi_gpu_reduce_op_sm90(int device) {
    using Variant = comm::TmaPipelineVariant<ChunkBytes, StageDepth>;
...
cudaError_t dispatch_multi_gpu_rank_kernel_variant_sm90(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    int peer_count,
    ...
    if (dtype == OO_DTYPE_FLOAT16) {
        if (op == OO_REDUCE_ADD || op == OO_REDUCE_SUM) {
            return launch_multi_gpu_reduce_op_sm90<
...
cudaError_t dispatch_multi_gpu_rank_kernel_sm90(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    int peer_count,
...
#define OO_TRY_VARIANT(CHUNK_BYTES_VALUE, STAGE_DEPTH_VALUE)                 \
    if (launch_config.chunk_bytes == (CHUNK_BYTES_VALUE) &&                  \
        ...
        return dispatch_multi_gpu_rank_kernel_variant_sm90<                  \
            (CHUNK_BYTES_VALUE),                                             \
            (STAGE_DEPTH_VALUE)>(                                            \
                local_in,                                                    \
                local_buf,                                                   \
...
template <int ChunkBytes, int StageDepth>
void configure_multi_gpu_dispatch_variant_sm90(
    oo_dtype_t dtype,
    oo_reduce_op_t op,
    int device) {
    if (dtype == OO_DTYPE_FLOAT16) {
        if (op == OO_REDUCE_ADD || op == OO_REDUCE_SUM) {
            configure_multi_gpu_reduce_op_sm90<
                comm::pipeline::PipelineReduceAddNoFtzF16,
                static_cast<int>(sizeof(half)),
                ChunkBytes,
                StageDepth>(device);
    ...
    throw std::invalid_argument(
        "tma_multi_gpu_allreduce_configure_kernel_once: unsupported dtype/op");
}

void configure_multi_gpu_dispatch_sm90(
    oo_dtype_t dtype,
    oo_reduce_op_t op,
    int device) {
    configure_multi_gpu_dispatch_variant_sm90<
...
void tma_multi_gpu_allreduce_configure_kernel_once(
    ...
    oo_reduce_op_t op,
    int device) {
    configure_multi_gpu_dispatch_sm90(dtype, op, device);
}
...
cudaError_t enqueue_tma_multi_gpu_allreduce_rank_sm90(
    ...
    try {
        return dispatch_multi_gpu_rank_kernel_sm90(
            local_in,
            local_buf,
            peer_bufs,
            peer_count,
