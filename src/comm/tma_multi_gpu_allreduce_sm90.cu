...
template <
    ...
    int FillDepth = StageDepth / 2,
    int LoadFillDepth = FillDepth>
cudaError_t launch_allreduce_rank_variant_sm90_transfer_plan(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    const int* peer_ranks,
...
template <
    ...
    int FillDepth = StageDepth / 2,
    int LoadFillDepth = FillDepth>
cudaError_t dispatch_allreduce_dtype_op_sm90_transfer_plan(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    const int* peer_ranks,
    ...
    if (dtype == OO_DTYPE_FLOAT16) {
        if (op == OO_REDUCE_ADD || op == OO_REDUCE_SUM) {
            return launch_allreduce_rank_variant_sm90_transfer_plan<
                comm::pipeline::PipelineReduceAddNoFtzF16,
                static_cast<int>(sizeof(half)),
                ChunkBytes,
                StageDepth,
...
cudaError_t dispatch_allreduce_variant_sm90_transfer_plan(
    const void* local_in,
    void* local_buf,
    void* const* peer_bufs,
    const int* peer_ranks,
...
#define OO_TRY_VARIANT(CHUNK_BYTES_VALUE, STAGE_DEPTH_VALUE, FILL_DEPTH_VALUE, LOAD_FILL_DEPTH_VALUE) \
    if (launch_config.chunk_bytes == (CHUNK_BYTES_VALUE) &&                                      \
        launch_config.stage_depth == (STAGE_DEPTH_VALUE)) {                                      \
        return dispatch_allreduce_dtype_op_sm90_transfer_plan<                                    \
            (CHUNK_BYTES_VALUE),                                                                 \
            (STAGE_DEPTH_VALUE),                                                                 \
            (FILL_DEPTH_VALUE),                                                                  \
            (LOAD_FILL_DEPTH_VALUE)>(                                                           \
...
cudaError_t enqueue_tma_multi_gpu_allreduce_rank_sm90_transfer_plan(
    ...
    try {
        return dispatch_allreduce_variant_sm90_transfer_plan(
            local_in,
            local_buf,
            peer_bufs,
            peer_ranks,
