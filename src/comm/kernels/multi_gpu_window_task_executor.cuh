 ...
 *
 * Dedicated by-value executor used only by the all-reduce experiment. Keep the
 * existing pointer kernel for all-gather/reduce-scatter.
 *
 * __grid_constant__ prevents nvcc from materializing a private per-thread copy
 * when plan.tasks is passed by address to execute_window_task_stripe().
 */
template <
    typename ReduceApply,
    int ChunkBytes,
    ...
    int LoadFillDepth = FillDepth,
    int SmallTaskBytes = TMA_TWO_GPU_PEER_SMALL_TASK_BYTES>
__global__ void multi_gpu_window_task_executor_kernel_by_value_sm90(
 ...
 * OOVERLAP_ALLREDUCE_PLAN_BY_VALUE_EXPERIMENT_V1
 *
 * Barrier-only cache for the by-value all-reduce path. Unlike
 * WindowPlanMappedScratch, this allocates no mapped plan and no CUDA event.
 */
...
inline cudaError_t get_cached_cta_barrier_scratch(
    ...
    *out = CtaBarrierScratch{};
    ...
    if (storage_index < 0 ||
        storage_index >= kOoMappedWindowPlanScratchSlots) {
        return cudaErrorInvalidValue;
    }

    static std::mutex mutex;
    static unsigned int* counters
        [32][kOoMappedWindowPlanScratchSlots] = {};
    static unsigned int last_values
        [32][kOoMappedWindowPlanScratchSlots] = {};
...
template <
    ...
    int LoadFillDepth = FillDepth,
    int SmallTaskBytes = TMA_TWO_GPU_PEER_SMALL_TASK_BYTES>
cudaError_t launch_multi_gpu_window_task_executor_by_value_sm90(
    const comm::plan::WindowTaskExecutorPlan<MaxTasks>& window_plan,
    int num_blocks,
    int threads,
    size_t dynamic_shared_bytes,
    ...
    multi_gpu_window_task_executor_kernel_by_value_sm90<
        ReduceApply,
        ChunkBytes,
        StageDepth,
        MaxTasks,
...
template <
    ...
    int LoadFillDepth = FillDepth,
    int SmallTaskBytes = TMA_TWO_GPU_PEER_SMALL_TASK_BYTES>
void configure_multi_gpu_window_task_executor_by_value_once(
    int device,
    ...
    if (total_smem_bytes >
        ...
        system::runtime::check_cuda(
            ...
            "cudaFuncSetAttribute(by-value MaxDynamicSharedMemorySize)");
    }
    ...
    system::runtime::check_cuda(
        cudaFuncSetAttribute(
            ...
            100),
        "cudaFuncSetAttribute(by-value PreferredSharedMemoryCarveout)");
